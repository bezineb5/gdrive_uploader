...
class MotionUploader:
    ...
    def _get_subfolder_id(self, root_folder_id, sub_folder):
        """Find and return the id of the folder given the title."""
        files = self.drive_service.files().list(q="title='%s' and mimeType contains 'application/vnd.google-apps.folder' and trashed=false and '%s' in parents" % (sub_folder, root_folder_id)).execute()
        if len(files['items']) == 1:
            folder_id = files['items'][0]['id']
            return folder_id
        else:
            raise Exception('Could not find the %s folder' % sub_folder)
